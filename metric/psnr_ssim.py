...
def calculate_ssim(img1,
    ...
    ssims = []
    # print('img1',img1.shape,img2.shape)
...
def cal_psnr_ssim_tOF_CVCP(save_path, res_vid_name, gt_vid_name, gt_path, one_video=True):
    my_filepaths = gt_path
    seq_ave_psnr = 0
    seq_ave_ssim = 0
    seq_ave_tOF = 0
...
def cal_psnr_ssim_tOF_REDS(save_path, res_vid_name, gt_vid_name, gt_path, one_video=True):
    my_filepaths = gt_path
    seq_ave_psnr = 0
    seq_ave_ssim = 0
    seq_ave_tOF = 0
    for res_f, gt_f in zip(res_vid_name, gt_vid_name):
        frames = 100 # int(res_f[-8:-5])
        psnr = 0
        ssim = 0
        tOF = 0
