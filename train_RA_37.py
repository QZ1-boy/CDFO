...
# ... 
from opt.data_RA_bi import CDVL_sideInfo_Dataset, RandomCrop, ToTensor, Augment
...
def parse_args():
    ...
    parser.add_argument('--val_itv',  default=400, type=int)
    parser.add_argument('--weight_decay', default=0.00001, type=float)
    parser.add_argument('--model_name', default=sys.argv[0][:-3], type=str)
    parser.add_argument('--qp', default=37,type=int)  
    return parser.parse_args()



def eval_seq(tst_list, gt_list, epoch, methods_name = "J", coding_cfg = "RA", testing=True, cal_metric=True):
    
    if testing:
        # for QP in (args.qp):  #  ['37', '37', '37', '37']  ['37']
        QP = args.qp
        model = CVSR_V8(SCGs=8)
        print("number of model parameters:", sum([np.prod(p.size()) for p in model.parameters()]))
        # sys.exit(0)
        # model_path = './training_results/train_RA_large_c_%s/ckpt/epoch-%s.pth' % (QP, coding_cfg, QP, methods_name, epoch)
        model_path = './training_results/train_RA_large_c_our_V8_37_%s/ckpt/epoch-%s.pth' % (args.qp, epoch)
        ...
        for img_set in tst_list:
            ...
            save_path = './train_evl_results/%s_QP%s_%s_V8_37/%s/' % (coding_cfg, QP, methods_name, img_set)
            ...
            for _, _, f in os.walk(tmp_path):
                ...
                for i in range(len(f)):
                    ...
                    # pm_Y = generate_DSF_input(o_list, tmp_side_path37 + 'DSF_Map/')
                    # pms = torch.unsqueeze(torch.cat(pm_Y, 0).to(device), 0)
    ...
    if cal_metric:
        f1 = open("./train_evl_results/log/%s_%s_our_V8_37_%s.txt" % (coding_cfg, methods_name, QP), 'a+')
        for one_t, one_gt in zip(tst_list, gt_list):
            psnr_s = []
            ssim_s = []
            
            # for QP in str(args.qp):  #  ['37', '37', '37', '37']  [ '37']
            QP = args.qp
            psnr, ssim, psnr_n, ssim_n = cal_psnr_ssim(
                        './train_evl_results/%s_QP%s_%s_V8_37/' % (coding_cfg, QP, methods_name),
                        [one_t],
                        [one_gt],
                        './test_data/gt_Y/')
            psnr_s.append(psnr)
            ssim_s.append(ssim)
            f1.write('# Epoch: %s M(%s_%s) Seq(%s) [QP37] PSNR/SSIM:\n' % (epoch, coding_cfg, methods_name, one_t))
            for p_i in psnr_s:
                print(p_i)
                f1.write(p_i + '\n')
            for s_i in ssim_s:
...
def train(args, model):
    ...
    side_dataset = CDVL_sideInfo_Dataset(csv_file='./misc/sequences_list.csv',
                                         ...
                                         need_bi_flag=False,
                                         mv_flag=True,
                                         HR_dir="/share4/home/zqiang/CVCP/Uncompressed_HR/",
                                         LR_dir_prefix="/share4/home/zqiang/CVCP/Decoded_LR/RA/",
                                         SideInfo_dir_prefix="/share4/home/zqiang/CVCP/Coding_Priors/RA/"
                                         )
    ...
    important_str += "*** " + arch_file_name
    important_str += "*** " + data_type_name
    important_str += "*"*20 + "\n"
    ...
    for epoch in range(args.warm_start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.val_itv == 0:
            ...
            psnr_s, ssim_s = eval_seq(res_vid_name, gt_vid_name, methods_name = "J_L_3x3", coding_cfg = "RA", testing=True, cal_metric=True, epoch=epoch+1)
...
def main(args):
    ...
    # model.load_state_dict(torch.load('/share4/home/zqiang/CVSR_train/training_results/train_RA_large_c_our_V8_37_37/ckpt/epoch-1600.pth', map_location='cpu'))
